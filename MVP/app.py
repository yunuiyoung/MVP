...
def estimate_development_effort(new_project_description: str):
    ...
    **요청 사항:**
    ...
    3. 각 파트별 공수(MM)는 반드시 아래의 key 이름(KOS_Order_MM, KOS_Con_MM, APILink_MM, Bill_MM, APP_MM, MEIN_MM)만 사용하여 정수 값으로 예측해주세요. **절대로 다른 key 이름을 사용하지 마세요.**
...
if __name__ == "__main__":
    ...
    if st.button("공수 예측 실행"):
        ...
        if predicted:
            ...
            df = {k: v for k, v in predicted.items() if k != "reason"}
            
            table = pd.DataFrame(list(df.items()), columns=["개발 파트", "예측 공수 (MM)"])
            st.table(table)
